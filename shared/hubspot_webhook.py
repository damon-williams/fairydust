...
async def send_hubspot_webhook(
    ...
    if not webhook_url or not webhook_enabled:
        logger.debug(f"HubSpot webhook skipped - URL: {bool(webhook_url)}, Enabled: {webhook_enabled}")
    ...
    payload = {
        ...
        "birth_date": user_data.get("birth_date").isoformat() if user_data.get("birth_date") else None,
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(webhook_url, json=payload) as response:
                if response.status == 200:
                    logger.info(f"HubSpot webhook sent successfully: {event_type} for user {user_data.get('fairyname')}")
                    return True
                else:
                    logger.warning(f"HubSpot webhook failed with status {response.status}: {event_type} for user {user_data.get('fairyname')}")
                    return False
                    
    except asyncio.TimeoutError:
        logger.warning(f"HubSpot webhook timeout: {event_type} for user {user_data.get('fairyname')}")
        return False
    except Exception as e:
        logger.warning(f"HubSpot webhook error: {e} for {event_type} user {user_data.get('fairyname')}")
        return False
