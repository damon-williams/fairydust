...
def validate_token_counts(prompt_tokens: int, completion_tokens: int, total_tokens: int) -> bool:
    ...
    if total_tokens != prompt_tokens + completion_tokens:
        logger.error(f"Token count mismatch: {total_tokens} != {prompt_tokens} + {completion_tokens}")
        return False
    ...
    if any(count > max_reasonable_tokens for count in [prompt_tokens, completion_tokens]):
        logger.warning(f"Unusually high token count: prompt={prompt_tokens}, completion={completion_tokens}")
    ...
    try:
        ...
        for provider, models in new_config.items():
            ...
            for model_id, pricing in models.items():
                ...
                if not isinstance(pricing["input"], (int, float)) or not isinstance(pricing["output"], (int, float)):
                ...
                if len(parts) >= 4:
                    ...
                    if provider in PRICING_CONFIG and model in PRICING_CONFIG[provider]:
                        if rate_type in ["input", "output"]:
                            ...
                            logger.info(f"Pricing override: {provider}/{model}/{rate_type} = {value}")
