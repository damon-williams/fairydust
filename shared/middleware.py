...
class RequestValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            return self._create_error_response(
                status_code=e.status_code, message=e.detail, details=getattr(e, "details", None)
            )
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if request.url.path.startswith("/api") or "json" in response.headers.get(
            "content-type", ""
        ):
...
def add_middleware_to_app(
    ...
    app.add_middleware(
        RequestValidationMiddleware, max_request_size=max_request_size, log_requests=log_requests
    )
