...
async def log_llm_usage(
    ...
    try:
        ...
        print(f"📊 LLM_USAGE: Logging usage for {app_id} - {model_id} ({total_tokens} tokens, ${cost_usd:.6f})", flush=True)
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code == 201:
                ...
                print(f"⚠️ LLM_USAGE: Failed to log usage - HTTP {response.status_code}: {response.text}", flush=True)
                return False
