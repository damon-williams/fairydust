...
def safe_json_parse(
    value: Any, default: T = None, expected_type: type = None
    ...
    try:
        ...
        if expected_type and not isinstance(parsed, expected_type):
            logger.warning(
                f"Parsed JSON type {type(parsed)} doesn't match expected {expected_type}"
            )
            return default
...
def parse_jsonb_field(field_value: Any, default: dict = None, field_name: str = "unknown") -> dict:
    """
...
def parse_recipe_metadata(recipe_data: dict) -> dict:
    ...
    metadata = parse_jsonb_field(
        recipe_dict.get("metadata"), default={}, field_name="recipe_metadata"
    )
...
def parse_story_data(story_data: dict) -> dict:
    ...
    characters = parse_jsonb_field(
        story_dict.get("characters_involved"), default=[], field_name="characters_involved"
    )
    story_dict["characters_involved"] = characters

    # Parse metadata JSONB field
    metadata = parse_jsonb_field(
        story_dict.get("metadata"), default={}, field_name="story_metadata"
    )
...
STORY_CHARACTER_SCHEMA = {
    ...
    "types": {"name": str, "relationship": str, "age": int, "traits": list},
}

USER_PROFILE_SCHEMA = {
    "required": ["field_name", "field_value"],
    "types": {"field_name": str, "category": str, "confidence_score": float, "source": str},
}
