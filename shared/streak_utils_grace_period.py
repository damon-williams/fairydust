...
async def calculate_daily_streak_for_auth(
    ...
    print(f"⏰ STREAK_DEBUG [{user_id}]: Grace period: {STREAK_GRACE_PERIOD_HOURS} hours", flush=True)
    ...
    if hours_since < 24:
        ...
        print(f"💔 STREAK_DEBUG [{user_id}]: BEYOND GRACE PERIOD ({hours_since:.1f} > {STREAK_GRACE_PERIOD_HOURS} hours) - Streak broken!", flush=True)
        new_streak = 1
        is_bonus_eligible = True
...
async def update_daily_streak_for_grant(
    ...
    if hours_since < 24:
        ...
        new_streak = current_streak + 1
        print(f"🔥 STREAK_UPDATE_DEBUG [{user_id}]: Within grace period - Incrementing to {new_streak}", flush=True)
    else:
        # Beyond grace period - reset
        new_streak = 1
        print(f"💔 STREAK_UPDATE_DEBUG [{user_id}]: Beyond grace period - Resetting to 1", flush=True)
