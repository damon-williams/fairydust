...
def test_retry_scenarios():
    ...
    status1 = StoryImageStatus(status="generating", attempt_number=1, max_attempts=3)
    print("📊 Scenario 1 - First attempt:")
    print(f"   Status: {status1.status}")
    print(f"   Attempt: {status1.attempt_number}/{status1.max_attempts}")
    print("   Front-end display: 'Generating... (attempt 1/3)'")
    print()

    # Scenario 2: Retry due to NSFW detection
    status2 = StoryImageStatus(
        status="retrying", attempt_number=2, max_attempts=3, retry_reason="nsfw"
    )
    ...
    print("   Front-end display: 'Retrying... (attempt 2/3) - Adjusting content'")
    print()

    # Scenario 3: Retry due to Replicate error
    status3 = StoryImageStatus(
        status="retrying", attempt_number=3, max_attempts=3, retry_reason="replicate_error"
    )
    ...
    print("   Front-end display: 'Retrying... (attempt 3/3) - Service error'")
    print()
    ...
    print("   Front-end display: 'Completed (took 2 attempts)'")
    print()
