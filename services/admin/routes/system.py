...
async def get_system_config_value(
    key: str, admin_user: dict = Depends(get_current_admin_user), db: Database = Depends(get_db)
):
...
async def update_system_config_value(
    ...
    if key == "daily_login_bonus_amount":
        try:
            ...
            if bonus_value < 0:
                raise HTTPException(
                    status_code=400, detail="Daily login bonus must be non-negative"
                )
        except ValueError:
        ...
        try:
            ...
            if initial_value < 0:
                raise HTTPException(
                    status_code=400, detail="Initial dust amount must be non-negative"
                )
        except ValueError:
            raise HTTPException(
                status_code=400, detail="Initial dust amount must be a valid integer"
            )

    # Check if key exists
    existing = await db.fetch_one("SELECT key FROM system_config WHERE key = $1", key)
...
async def get_public_system_config_value(key: str, db: Database = Depends(get_db)):
    ...
    config = await db.fetch_one("SELECT value FROM system_config WHERE key = $1", key)
    if not config:
