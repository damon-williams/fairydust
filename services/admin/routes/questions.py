...
async def questions_list(
    ...
    categories = await db.fetch_all(
        "SELECT DISTINCT category FROM profiling_questions ORDER BY category"
    )
    ...
    for cat in categories:
        ...
        category_options += (
            f'<option value="{cat["category"]}" {selected}>{cat["category"].title()}</option>'
        )
...
async def new_question_form(request: Request, admin_user: dict = Depends(get_current_admin_user)):
    return HTMLResponse(
        f"""
    <!DOCTYPE html>
    <html>
...
async def edit_question_form(
    ...
    question = await db.fetch_one("SELECT * FROM profiling_questions WHERE id = $1", question_id)
...
async def toggle_question(
    ...
    question = await db.fetch_one(
        "SELECT is_active FROM profiling_questions WHERE id = $1", question_id
    )
    if not question:
        raise HTTPException(status_code=404, detail="Question not found")
    ...
    await db.execute(
        "UPDATE profiling_questions SET is_active = $1 WHERE id = $2", new_status, question_id
    )
...
async def delete_question(
    ...
    response_count = await db.fetch_one(
        "SELECT COUNT(*) as count FROM user_question_responses WHERE question_id = $1", question_id
    )
