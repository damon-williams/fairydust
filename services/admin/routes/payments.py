...
async def get_verification_failures(
    ...
    try:
        ...
        return {"failures": failures_list, "total": len(failures_list), "days": days}

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch verification failures: {str(e)}"
        )
...
async def refund_transaction(transaction_id: str, reason: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to connect to ledger service: {str(e)}"
        )
    except Exception as e:
