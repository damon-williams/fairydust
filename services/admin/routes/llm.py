...
async def llm_app_config(
    ...
    if not config:
        ...
        if app["slug"] == "fairydust-recipe":
            ...
            default_cost_limits = (
                '{"per_request_max": 0.15, "daily_max": 25.0, "monthly_max": 200.0}'
            )
    ...
    primary_parameters = parse_json_field(
        config["primary_parameters"], {"temperature": 0.8, "max_tokens": 150, "top_p": 0.9}
    )
    fallback_models = parse_json_field(config["fallback_models"], [])
    cost_limits = parse_json_field(
        config["cost_limits"], {"per_request_max": 0.05, "daily_max": 10.0, "monthly_max": 100.0}
    )
    feature_flags = parse_json_field(
...
async def update_llm_app_config(
    ...
    try:
        ...
        return RedirectResponse(url=f"/admin/llm/apps/{app_id}?updated=1", status_code=302)

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error updating configuration: {type(e).__name__}: {str(e)}"
        )

    return RedirectResponse(url=f"/admin/llm/apps/{app_id}?error=1", status_code=302)
