...
async def apps_list(
    ...
    for builder in builders:
        builder_options += f'<option value="{builder["id"]}">{builder["fairyname"]} ({builder["email"]})</option>'
...
async def create_app(
    ...
    try:
        ...
        builder = await db.fetch_one(
            "SELECT id, fairyname FROM users WHERE id = $1 AND is_builder = true",
            builder_id
        )
        if not builder:
            return RedirectResponse(
                url="/admin/apps?error=Invalid builder selected",
                status_code=302
            )
        
        # Check if slug already exists
        existing_app = await db.fetch_one(
            "SELECT id FROM apps WHERE slug = $1",
            slug
        )
        if existing_app:
            return RedirectResponse(
                url="/admin/apps?error=App slug already exists",
                status_code=302
            )
        ...
        if dust_per_use < 1 or dust_per_use > 100:
            return RedirectResponse(
                url="/admin/apps?error=DUST per use must be between 1 and 100",
                status_code=302
            )
        ...
        """, 
            app_id, uuid.UUID(builder_id), name, slug, description, 
        ...
        return RedirectResponse(
            url="/admin/apps?error=Failed to create app. Please try again.",
            status_code=302
        )