...
async def activate_terms_document(
    ...
    try:
        ...
        document = await db.fetch_one(
            "SELECT * FROM terms_documents WHERE id = $1",
            document_id
        )
        ...
        await db.execute(
            "UPDATE terms_documents SET is_active = true WHERE id = $1",
            document_id
        )
...
async def deactivate_terms_document(
    ...
    try:
        result = await db.execute(
            "UPDATE terms_documents SET is_active = false WHERE id = $1",
            document_id
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to deactivate terms document: {str(e)}")
...
async def get_terms_stats(
    ...
    try:
        ...
        if user_count["total_users"] > 0:
            compliance_rate = (users_with_acceptances["users_with_acceptances"] / user_count["total_users"]) * 100
