...
class BuilderAuth:
    ...
    async def create_builder_session(self, user_id: str, fairyname: str) -> str:
        ...
        await self.redis.setex(
            f"builder_session:{user_id}", BUILDER_SESSION_EXPIRE_HOURS * 3600, token
        )
...
async def get_current_builder_user(
    request: Request, builder_session: Optional[str] = Cookie(None), db: Database = Depends(get_db)
) -> dict:
    ...
    user = await db.fetch_one(
        "SELECT * FROM users WHERE id = $1 AND is_active = true", session_data["user_id"]
    )
