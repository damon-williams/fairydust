...
class StoryImageService:
    ...
    def extract_image_scenes(
        self, story_content: str, story_length: StoryLength, characters: list[StoryCharacter]
    ) -> list[dict]:
        """Extract key scenes from story for image generation with proper image count"""

        image_count = IMAGE_COUNTS[story_length]
        print(
            f"🎨 IMAGE_EXTRACTION: Expected {image_count} images for {story_length.value} story",
            flush=True,
        )
        ...
        if len(paragraphs) < image_count:
            ...
            print(
                f"⚠️ IMAGE_EXTRACTION: Only {len(paragraphs)} paragraphs available, using all of them",
                flush=True,
            )
        else:
            ...
            print(
                f"✅ IMAGE_EXTRACTION: Distributing {image_count} images at positions: {placement_points}",
                flush=True,
            )
            ...
            scenes.append(
                {
                    ...
                    "characters_mentioned": self._identify_characters_in_scene(
                        scene_context, characters
                    ),
                }
            )

            print(
                f"🎨 IMAGE_EXTRACTION: Created scene {i+1}/{len(placement_points)} at position {point} with ID {image_id}",
                flush=True,
            )
    ...
    def insert_image_markers(self, story_content: str, scenes: list[dict]) -> str:
        ...
        print(
            f"🖼️ IMAGE_MARKERS: Inserting {len(scenes)} image markers into {len(paragraphs)} paragraphs",
            flush=True,
        )
        ...
        for scene in sorted_scenes:
            ...
            if position < len(paragraphs):
                print(
                    f"🖼️ IMAGE_MARKERS: Inserting {scene['image_id']} after paragraph {position}",
                    flush=True,
                )
                paragraphs[position] += image_marker
            else:
                print(
                    f"⚠️ IMAGE_MARKERS: Position {position} is out of range for {len(paragraphs)} paragraphs",
                    flush=True,
                )
    ...
    async def generate_image_prompt(
        ...
        # Use the sophisticated 4-agent system for prompt generation with enhanced context
        return await multi_agent_image_service.generate_image_prompt(
            ...
            full_story_content=full_story_content,
        )
    ...
    def _build_explicit_character_description(self, char: StoryCharacter) -> str:
        ...
        if char.relationship and len(char.relationship.split()) > 2:
            ...
            if any(
                desc_word in char.relationship.lower()
                for desc_word in ["helps", "magical", "who", "that", "with", "made of"]
            ):
    ...
    def prepare_reference_people(
        self, characters_in_scene: list[StoryCharacter], user_id: str
        ...
        for character in characters_in_scene[:3]:  # Limit to 3 for Gen-4 model
            ...
            if character.photo_url:
                print(
                    f"📸 STORY_IMAGE: Using photo reference for {character.name} - {character.photo_url}"
                )
                ...
                print(
                    f"📝 STORY_IMAGE: No photo available for {character.name} - will use text description only"
                )

        print(
            f"📸 STORY_IMAGE: Prepared {len(reference_people)} photo references out of {len(characters_in_scene)} characters"
        )
        return reference_people

    def select_most_important_characters(
        self, characters: list[StoryCharacter], limit: int = 3
    ) -> list[StoryCharacter]:
