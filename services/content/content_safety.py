...
class ContentSafetyFilter:
    ...
    def validate_request(self, request: StoryGenerationRequest) -> tuple[bool, list[str]]:
        ...
        if request.target_audience == TargetAudience.CHILD:
            ...
            if adult_themes:
                issues.extend(
                    [f"Adult theme detected for child audience: {theme}" for theme in adult_themes]
                )

        return len(issues) == 0, issues

    def filter_generated_content(
        self, content: str, target_audience: TargetAudience
    ) -> tuple[str, list[str]]:
        ...
        if inappropriate_found:
            warnings.extend(
                [f"Inappropriate content detected: {word}" for word in inappropriate_found]
            )
