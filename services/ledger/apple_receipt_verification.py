...
class AppleReceiptVerificationService:
    ...
    async def verify_receipt(self, receipt_data: str, product_id: str) -> Tuple[bool, Dict, Optional[str]]:
        ...
        print(f"🍎 APPLE_RECEIPT: Primary verification complete. Status: {status_code}, Valid: {is_valid}")
        ...
        if is_valid:
            print(f"🍎 APPLE_RECEIPT: ✅ Receipt verified with Apple. Running content validation...")
            ...
            print(f"🍎 APPLE_RECEIPT: ✅ Content validation passed")
        else:
    ...
    def _validate_receipt_contents(self, response: Dict, expected_product_id: str) -> Optional[str]:
        ...
        for purchase in in_app_purchases:
            if purchase.get("product_id") == expected_product_id:
                ...
                print(f"🍎 APPLE_RECEIPT: ✅ Found matching purchase: transaction_id={purchase.get('transaction_id')}, quantity={purchase.get('quantity', 1)}")
                break
        ...
        print(f"🍎 APPLE_RECEIPT: Transaction details - ID: {transaction_id}, Original: {original_transaction_id}, Date: {purchase_date_ms}, Quantity: {quantity}")
        ...
        print(f"🍎 APPLE_RECEIPT: ✅ Receipt content validation passed")
        return None  # Valid
    ...
    def extract_transaction_data(self, verification_response: Dict, product_id: str) -> Dict:
        ...
        for purchase in in_app_purchases:
            if purchase.get("product_id") == product_id:
                matching_purchase = purchase
                print(f"🍎 APPLE_RECEIPT: ✅ Found matching purchase for extraction")
                break
