...
from models import (
    TransactionType, TransactionStatus, Transaction,
    TransactionResponse, CachedBalance
)
...
class LedgerService:
    ...
    async def get_balance(self, user_id: UUID, use_cache: bool = True) -> int:
        ...
        result = await self.db.fetch_one(
            "SELECT dust_balance FROM users WHERE id = $1",
            user_id
        )
    ...
    async def consume_dust(
        ...
        if existing_tx_id:
            ...
            tx = await self.db.fetch_one(
                "SELECT * FROM dust_transactions WHERE id = $1",
                existing_tx_id
            )
            if tx:
        ...
        try:
            ...
            async with self.db.transaction() as conn:
                ...
                user = await conn.fetchrow(
                    "SELECT id, dust_balance FROM users WHERE id = $1 FOR UPDATE",
                    user_id
                )
                ...
                transaction = await conn.fetchrow(
                    ...
                    RETURNING *
                    """,
                    transaction_id, user_id, -amount, TransactionType.CONSUME.value,
                    TransactionStatus.COMPLETED.value, f"Consumed for {action}",
                    app_id, json.dumps(metadata) if metadata else None, idempotency_key
                )
        ...
        try:
            async with self.db.transaction() as conn:
                ...
                transaction = await conn.fetchrow(
                    ...
                    RETURNING *
                    """,
                    transaction_id, user_id, amount, TransactionType.GRANT.value,
                    TransactionStatus.COMPLETED.value, reason,
                    json.dumps({**(metadata or {}), "admin_id": str(admin_id)}) if admin_id else json.dumps(metadata) if metadata else None
                )
    ...
    async def refund_transaction(
        ...
        existing_refund = await self.db.fetch_one(
            ...
            AND type = $2
            """,
            str(transaction_id), TransactionType.REFUND.value
        )
