...
def main():
    ...
    parser.add_argument(
        "--expires-years", type=int, default=10, help="Token expiry in years (default: 10)"
    )
    parser.add_argument(
        "--no-expiry", action="store_true", help="Generate token with no expiration"
    )
    ...
    try:
        ...
        if expires_years > 0:
            ...
            print(
                f"Expires: {expire_date.strftime('%Y-%m-%d %H:%M:%S')} UTC ({expires_years} years)"
            )
        else:
            print("Expires: Never")
        print("=" * 60)
